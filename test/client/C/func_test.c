...
int main(int argc, char** argv)
{
    ...
    cfio_init(comm, LAT_PROC, LON_PROC, ratio);
    CFIO_START(rank);
    ...
    cfio_put_att(ncidp, NC_GLOBAL, "test1", CFIO_FLOAT, 1, &b);
    cfio_put_att(ncidp, NC_GLOBAL, "test2", CFIO_CHAR, strlen(test), test);

    cfio_def_var(ncidp,"time_v", CFIO_DOUBLE, 0, dimids,  &var1);
    //printf("inq res %d %d\n", cfio_inq_att(ncidp, NC_GLOBAL, "test"), cfio_inq_att(ncidp, var1, "test"));
    cfio_put_att(ncidp, var1, "test", CFIO_CHAR, strlen(test), test);
    printf("inq res %d %d\n", cfio_inq_att(ncidp, NC_GLOBAL, "test"), cfio_inq_att(ncidp, var1, "test"));

        int ndims;
        cfio_id_inq_varndims(ncidp, var1, &ndims);

        int dimids_inq[2];
        cfio_id_inq_vardimid(ncidp, var1, dimids_inq);
    cfio_enddef(ncidp);
    if(0 != rank) {
        start[0] = start[1] = count[0] = count[1] = 0;
    }
    cfio_put_vara_double(ncidp,var1, 0,start, count,fp); 
    cfio_close(ncidp);
    cfio_io_end();
    free(fp);
