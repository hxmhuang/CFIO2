...
static inline void ensure_free_space(cfio_buf_t *buf_p, size_t size, void(*free)())
{
    ...
    if (buf_p->size < size) {
        printf("CFIO ERROR: BUFFER IS NOT ENOUGH! BUFFER SIZE = %lu, BUT MSG SIZE = %lu \n", buf_p->size, size);
    }
    
    while((free_size = free_buf_size(buf_p)) < size)
    {
        printf("%s, start = %lu, end = %lu, used = %lu, free = %lu, size = %lu \n", __func__, 
                buf_p->start_addr, buf_p->start_addr + buf_p->size, 
                buf_p->used_addr, buf_p->free_addr, size);
	free();
    }
