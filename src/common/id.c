...
static int _compare_att_name(struct qhash_head *link, void *key)
{
    assert(NULL != key);
    assert(NULL != link);

    cfio_id_att_t *att = qlist_entry(link, cfio_id_att_t, link);

    if(strcmp((char *)key, att->name) == 0)
    {
        return 1;
    }
...
int cfio_id_assign_nc(int *nc_id)
{
    ...
    *nc_id = open_nc_a;
    ...
    cfio_id_assign_var(open_nc_a, "NC_GLOBAL", 0, NULL, &global_varid);
...
int cfio_id_assign_var(int nc_id, char *var_name, int ndims, int *dimids, int *var_id)
{
    assert(var_id != NULL);
    ...
    link = qhash_search(assign_table, &key);
    if (NULL == link) {
        error("nc_id(%d) not found in assign_table.", nc_id);
        return CFIO_ERROR_NC_NO_EXIST;
    } else {
        nc_val = qlist_entry(link, cfio_id_val_t, hash_link);
        link = qlist_find(nc_val->var_head, _compare_client_name, var_name);
        if (NULL == link) {
            if(strcmp(var_name, "NC_GLOBAL") != 0) { 
                nc_val->client_var_a ++;
                *var_id = nc_val->client_var_a;
            } else {
                *var_id = NC_GLOBAL;
            }

            /// part 1:
            /// add var to list of nc_val
            name_entry = malloc(sizeof(cfio_id_client_name_t));
            if (NULL == name_entry) {
                error("malloc fail.");
                return CFIO_ERROR_MALLOC;
            ...
            qlist_add(&name_entry->link, nc_val->var_head);

            /// part 2:
            /// add a key(ncid,varid)-value(var) pair to hashtable
            key.client_var_id = *var_id;
            ...
            memset(val, 0, sizeof(cfio_id_val_t));
            val->client_nc_id = nc_id;
            val->client_var_id = *var_id;
            ...
            memset(val->var, 0, sizeof(cfio_id_var_t));
            INIT_QLIST_HEAD(&(val->link));
            val->var->name = strdup(var_name);
            val->var->nc_id = val->client_nc_id;
            val->var->var_id = val->client_var_id;
            val->var->ndims = ndims;
            ...
            if (0 != ndims) {
                val->var->dim_ids = malloc(sizeof(int) * ndims);
                if (NULL == val->var->dim_ids) {
                    error("malloc fail.");
                    return CFIO_ERROR_MALLOC;
                }
                memcpy(val->var->dim_ids, dimids, sizeof(int) * ndims);
            }
            ...
            *var_id = name_entry->id;
            ...
            /// TODO: update ndims/dimids or not?
        }
        debug(DEBUG_ID, "success return.");
        return CFIO_ERROR_NONE;
    }
...
int cfio_id_inq_dim(int nc_id, char *dim_name, int *dim_id)
{
    assert(dim_id != NULL);
...
int cfio_id_inq_att(int nc_id, int var_id, char *att_name)
{
    cfio_id_key_t key;
    cfio_id_val_t *val;
    cfio_id_var_t *var;
    ...
    }else
    {
        val = qlist_entry(link, cfio_id_val_t, hash_link);
        var = val->var;
        att_link = qlist_find(var->att_head, _compare_att_name, att_name);
        if(att_link == NULL)
        {
            return CFIO_ERROR_VAR_NO_EXIST;
        } 	
...
int cfio_id_inq_varndims(int nc_id, int var_id, int *ndims)
{
    cfio_id_key_t key;
    cfio_id_val_t *val;
    struct qhash_head *link;
    ...
    link = qhash_search(assign_table, &key);
    if (NULL == link) {
        error("nc_id(%d)&&var_id(%d) not found in assign_table.", nc_id, var_id);
        return CFIO_ERROR_NC_NO_EXIST;
    } else {
        val = qlist_entry(link, cfio_id_val_t, hash_link);
        *ndims = val->var->ndims;
...
int cfio_id_inq_vardimid(int nc_id, int var_id, int *dimids)
{
    cfio_id_key_t key;
    cfio_id_val_t *val = NULL;
    ...
    if (NULL == link) {
        ...
        val = qlist_entry(link, cfio_id_val_t, hash_link);
        ndims = val->var->ndims;
        memcpy(dimids, val->var->dim_ids, sizeof(int)*ndims);
        ...
        char *name, cfio_type xtype, int len, char *data)
{
    ...
    if(NULL == (link = qhash_search(assign_table, &key))) { 
        ...
        att->xtype = xtype;
        att->len = len;
        att->data = (char *)malloc(sizeof(char) * len);
...
void cfio_id_val_free(cfio_id_val_t *val)
{
    ...
    if(NULL != val)
    {
        ...
        if(NULL != val->var)
        {
            ...
            if(0 != val->var->ndims && NULL != val->var->dim_ids)
            {
