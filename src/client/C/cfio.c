...
int cfio_init(MPI_Comm _inter_comm, int x_proc_num, int y_proc_num, int ratio)
{
    int rc, i;
    int size;
    int root = 0;
    ...
    inter_comm = _inter_comm;

    MPI_Comm_size(inter_comm, &size);
    ...
    MPI_Comm_create(inter_comm, server_group, &server_comm);
    free(ranks);
    ...
    if(cfio_map_proc_type(rank) == CFIO_MAP_TYPE_SERVER)
    {
        ...
        //printf("MPI_Comm_World %d server_comm %d. \n", rank, server_comm_rank);
        ...
        int *varidp)
{
    ...
    if((ret = cfio_id_assign_var(ncid, name, ndims, dimids, varidp)) < 0)
    {
        error("");
        return ret;
    }
...
int cfio_inq_dimid(int ncid, char *dim_name, int *dimid)
{
    int ret;

    debug(DEBUG_CFIO, "nc_id = %d, dim_name = %s", ncid, dim_name);

    if((ret = cfio_id_inq_dim(ncid, dim_name, dimid)) < 0)
    {
        error("");
        return ret;
    }

    debug(DEBUG_CFIO, "get dimid = %d", *dimid);
    return CFIO_ERROR_NONE;
...
int cfio_inq_att(int ncid, int var_id, char *att_name)
{
    int ret;

    debug(DEBUG_CFIO, "nc_id = %d, att_name = %s", ncid, att_name);

    if((ret = cfio_id_inq_att(ncid, var_id, att_name)) < 0)
    {
        error("");
        return ret;
    }
...
int cfio_inq_varndims(int ncid, int varid, int *ndims)
{
    int ret;

    debug(DEBUG_CFIO, "nc_id = %d, var_id = %d", ncid, varid);

    if((ret = cfio_id_inq_varndims(ncid, varid, ndims)) < 0)
        debug(DEBUG_CFIO, "get ndims = %d", *ndims);
    return CFIO_ERROR_NONE;

}

int cfio_inq_vardimid(int ncid, int varid, int *dimids)
{
    int ret;

    debug(DEBUG_CFIO, "nc_id = %d, var_id = %d", ncid, varid);

    if((ret = cfio_id_inq_vardimid(ncid, varid, dimids)) < 0)
    {
        error("");
        return ret;
    }

    debug(DEBUG_CFIO, "get dimids");
    return CFIO_ERROR_NONE;
}
 ...
 *For Fortran Call
 **/
void cfio_init_c_(int *_inter_comm, int *x_proc_num, int *y_proc_num, int *ratio, int *ierr)
{
    *ierr = cfio_init(*_inter_comm, *x_proc_num, *y_proc_num, *ratio);
}
        ...
        int *ierr)
{
    ...
    *ierr = cfio_def_var(*ncid, name, *xtype, *ndims, _dimids, varidp);
...
{
    size_t *_start = NULL, *_count = NULL;
    int i, j;
...
void cfio_put_vara_text_c_(
        int *ncid, int *varid, int *ndims,
        int *start, int *count, char *fp, int *ierr)
{
    size_t *_start, *_count;
    int i, j;
...
void cfio_inq_dimid_c_(int *ncid, char *dim_name, int *name_len, int *dimid, int *ierr)
{
    char *name;

    name = malloc(*name_len + 1);
    snprintf(name, (*name_len + 1), "%s", dim_name);


    *ierr = cfio_inq_dimid(*ncid, name, dimid);
...
void cfio_inq_att_c_(int *ncid, int *varid, char *att_name, int *name_len, int *ierr)
{
    char *name;

    name = malloc(*name_len + 1);
    snprintf(name, (*name_len + 1), "%s", att_name);


    *ierr = cfio_inq_att(*ncid, *varid, name);
...
void cfio_inq_varndims_c_(int *ncid, int *varid, int *ndims, int *ierr)
{
    *ierr = cfio_inq_varndims(*ncid, *varid, ndims);

    return;
}

void cfio_inq_vardimid_c_(int *ncid, int *varid, int *dimids, int *ierr)
{
    *ierr = cfio_inq_vardimid(*ncid, *varid, dimids);
