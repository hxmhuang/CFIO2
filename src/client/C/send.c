...
static int itr = 1;
static double transfer_addr_time = 0.0;
...
static void cfio_send_client_buf_free()
{
    ...
    if (buffer->used_addr != addr.free_addr) {
...
static void cfio_send_client_buf_free_old()
{
#ifdef SEND_ADDR
    if (!cfio_rdma_addr_client_test(&addr)) {
        cfio_rdma_client_recv_ack();
        ...
        int varid)
{
    ...
    size += cfio_buf_str_size(name);
    size += cfio_buf_data_size(sizeof(cfio_type));
    size += cfio_buf_data_size(sizeof(int)); //ndims
    if (0 != ndims) {
        size += cfio_buf_data_array_size(ndims, sizeof(int)); //dimids
    }
    size += cfio_buf_data_size(sizeof(int));
    ...
    cfio_buf_pack_str(name, buffer);
    cfio_buf_pack_data(&xtype, sizeof(cfio_type), buffer);
    cfio_buf_pack_data(&ndims, sizeof(int), buffer);
    if (0 != ndims) {
        cfio_buf_pack_data_array(dimids, ndims, sizeof(int), buffer);
    }
    cfio_buf_pack_data(&varid, sizeof(int), buffer);
...
#ifdef TRANSFER_ADDR_TIME
    double temp_time = times_cur(); // transfer_addr_time
#endif
...
#ifdef TRANSFER_ADDR_TIME
    if (itr > 3) {
        transfer_addr_time += times_cur() - temp_time; // transfer_addr_time
    }
#endif
