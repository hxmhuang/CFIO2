...
static inline int _handle_def_old(cfio_id_val_t *val)
{
    int ret, i;
    cfio_id_nc_t *nc;
    cfio_id_dim_t *dim;
...
static inline int _handle_def(cfio_id_val_t *val)
{
    ...
    if (NULL != val->dim) {
        ...
        printf("%s, %d, [CFIO SERVER DEBUG] end def dim %s %d \n", __FILE__, __LINE__, dim->name, ret);
#else
        ret = NC_NOERR;
        dim->dim_id = 10;
#endif
    ...
    if (NULL != val->var) {
        ...
        printf("%s, %d, [CFIO SERVER DEBUG] end def var %s %d \n", __FILE__, __LINE__, var->name, ret);
#else
        ret = NC_NOERR;
        var->var_id = 10;
#endif
 ...
 */
void _update_start_and_count_old(int ndims, 
...
int cfio_io_def_var_old(cfio_msg_t *msg)
{
    int ret = 0, i;
    ...
    ret = cfio_recv_unpack_def_var_old(msg, &client_nc_id, &name, &xtype, &ndims, 
            ...
            cfio_id_get_var(client_nc_id, client_var_id, &var))
    {
         ...
         **/
        client_num = cfio_map_get_client_num_of_server(server_id);
        cfio_id_map_var_old(name, client_nc_id, client_var_id, 
    ...
    }else
    {
        ...
        _update_start_and_count_old(ndims, var->start, var->count, start, count);
        for(i = 0; i < ndims; i ++)
        {
...
int cfio_io_def_var(cfio_msg_t *msg)
{
    ...
    {
        /**
         * the fisrst def var msg arrive, not need to free the name, 
         * and client_dim_ids
         **/
        client_num = cfio_map_get_client_num_of_server(server_id);
        cfio_id_map_var(name, client_nc_id, client_var_id, 
                CFIO_ID_NC_INVALID, CFIO_ID_VAR_INVALID, 
                ndims, client_dim_ids, xtype, client_num);
    } else {
        /**
         *we need to free pointer 
         **/
        free(name);
        free(client_dim_ids);
    }
...
int cfio_io_put_vara(cfio_msg_t *msg)
{
    ...
    if(_bitmap_full(io_info->client_bitmap))
    {
        ...
        printf("%s, %d, [CFIO SERVER %d DEBUG] end put vara %s \n", __FILE__, __LINE__, server_id, var->name);
    }
