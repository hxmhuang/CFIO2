...
int cfio_recv_unpack_def_var_old(
        cfio_msg_t *msg,
        ...
        int *varid)
{
    ...
    cfio_buf_unpack_str(name, buffer[client_index]);
    cfio_buf_unpack_data(xtype, sizeof(cfio_type), buffer[client_index]);
    cfio_buf_unpack_data(ndims, sizeof(int), buffer[client_index]);
    if (0 != *ndims) { 
        cfio_buf_unpack_data_array((void **)dimids, ndims, sizeof(int), buffer[client_index]);
    }
    cfio_buf_unpack_data(varid, sizeof(int), buffer[client_index]);
